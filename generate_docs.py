...
base_dir = os.path.dirname(os.path.abspath(__header__)) if '__header__' in locals() else os.getcwd()
...
for pag in paginas:
    full_path = os.path.join(base_dir, pag["path"])
    os.makedirs(os.path.dirname(full_path), exist_ok=True)
    
    # Check if file has special content
    manual_pages = [
        "paginas/01-principal/dashboard.html",
        "paginas/02-operar/recebimento.html",
        "paginas/03-planejar/ondas.html",
        "paginas/04-controlar/inventario.html",
        "paginas/07-cadastrar/produtos.html",
        "paginas/mobile/picking.html"
    ]
    
    # If it's one of the manual pages, we skip overwriting it IF it already exists and has more than boilerplate
    # But since we need to fix encoding/links, we might as well rewrite them if we have the content
    # For now, let's just use the boilerplate for everything else
    if pag["path"] not in manual_pages:
        html = f"""<!DOCTYPE html>
<html lang="pt-br">
<head>
  <meta charset="UTF-8">
